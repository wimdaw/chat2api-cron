...
部署为 Koyeb Cron Job: 每天执行, 用 session cookie 拿新 token 并上传到 chat2api
...
def step1_check_current():
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as r:
            if r.status == 200:
                log("当前 token 仍有效(已有可用 token)")
                return True
    except urllib.error.HTTPError as e:
        if e.code == 401:
            log("当前 token 已失效(401), 需要刷新")
            return False
    except Exception as e:
        log(f"检查异常: {e}")
    return None

def step2_get_new_token():
    ...
    try:
        ...
        log(f"❌ session 请求失败 HTTP {e.code}")
        return None
    except Exception as e:
...
def step3_upload(at):
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as r:
            ...
            log(f"✅ 上传成功, chat2api 当前 token 数: {result.get('tokens_count','?')}")
            return True
    except Exception as e:
        log(f"❌ 上传失败: {e}")
        return False
...
def main():
    ...
    check = step1_check_current()
    if check is True:
        log("当前 token 有效, 仍执行续期(提前喂新 token 防过期)")
    new_at = step2_get_new_token()
    if not new_at:
